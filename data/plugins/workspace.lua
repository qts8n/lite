...
config.workspace_dir = EXEDIR.."/data/"


local function current_dir_to_workspace_path()
  local current_dir = system.absolute_path(".")
  local workspace_filename = current_dir:gsub("/", ".")
  workspace_filename = workspace_filename:gsub("\\", "-")
  workspace_filename = workspace_filename:gsub("%s+", "_")
  workspace_filename = workspace_filename:gsub(":", "")
  local workspace_dir = config.workspace_dir:gsub("%/+$", "")
  return workspace_dir.."/"..workspace_filename..".workspace.lua"
end
...
local function save_workspace()
  local workspace_path = current_dir_to_workspace_path()
  local root = get_unlocked_root(core.root_view.root_node)
  local fp = io.open(workspace_path, "w")
  if fp then
...
local function load_workspace()
  local workspace_path = current_dir_to_workspace_path()
  local ok, t = pcall(dofile, workspace_path)
